...
# Eliminamos lo que es exclusivo del laboratorio privado (Sanitización)
git rm -r --cached tests/ -f 2>$null              # Pruebas internas (Testing)
git rm -r --cached configs/ -f 2>$null             # Entornos y configs reales
git rm -r --cached scripts/ -f 2>$null             # Scripts de automatización interna
git rm -r --cached src/rules/custom_rules.yaml -f 2>$null # Payloads y lógica crítica interna
git rm --cached .gitlab-ci.yml -f 2>$null          # CI/CD interno del Lab
git rm -r --cached .github/workflows/ -f 2>$null   # Automatización extra sensible
